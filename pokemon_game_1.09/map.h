...
class World {
  public:
    ...
    World () {
      ...
      for (i = 0; i < Y_MAX; ++i) {
        for (j = 0; j < X_MAX; ++j) {
            this->hiker_cost[i][j] = 0;  // or any other suitable initial value
            this->rival_cost[i][j] = 0;  // or any other suitable initial value
        }
      }

      this->curr_idx[dim_y] = 200;
      this->curr_idx[dim_x] = 200;

      this->pc = PC();
      this->pc_turn.character = &this->pc;
      this->pc_turn.next_turn = 0;
      this->pc_turn.seq_num = 0;

      this->top_msg_win = newwin(1, X_MAX, 0, 0);
      keypad(this->top_msg_win, TRUE);
      this->bottom_msg_win = newwin(2, X_MAX, 22, 0);
      this->main_win = newwin(Y_MAX, X_MAX, 1, 0);
      ...
      for (i = 0; i < WRLD_MAX; i++) {
        for (j = 0; j < WRLD_MAX; j++) {
          if (this->world[i][j] != NULL) {
            for (t = 0; t < (int) (this->world[i][j]->num_trainers); t++) {
                if (this->world[i][j]->turn_table[t].character != NULL) 
                    free(this->world[i][j]->turn_table[t].character);

                if (this->world[i][j]->turn_table[t].hn != NULL) 
                    free(this->world[i][j]->turn_table[t].hn);
            }
            free(this->world[i][j]);
          }
        }
